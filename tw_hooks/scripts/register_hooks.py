#!/usr/bin/env python3
"""Detect Taskwarrior hooks and register callbacks for either all or some of them.

This program does not create a shim (a standalone python executable) on the filesystem for each
one of the hooks. Instead it always creates 4 executables - one per TW event (on-launch,
on-exit, on-add, on-modify).

See the TW Hooks API for more: https://taskwarrior.org/docs/hooks/

Upon invocation of one of these executables it then traverses and
calls each one of the callbacks of the register tw_hooks from inside python so as to avoid
calling them as from an external process).
"""
...
def main():
    ...
    parser.add_argument(
        "-a",
        "--all-hooks",
        help="Register callbacks for all the hooks in this repo",
        action="store_true",
    )
    parser.add_argument(
        "-l",
    ...
    # make sure there are hooks to install otherwise exit
    for _, subclasses in hooks_to_install.items():
        if subclasses:
            break
    else:
        logger.warning("No hooks to install.")
        return

    # install one shim per TW Hook event
